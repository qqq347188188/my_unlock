...
static NSString *LogPath(void) {
    ...
    dispatch_once(&o, ^{
        ...
        p = [d stringByAppendingPathComponent:@"一木记账.log"];
    });
    return p;
}
...
static BOOL isTargetURL(NSURL *u) {
    if (!u) return NO;
    if (![u.host.lowercaseString isEqualToString:@"yimuapp.com:8082"]) return NO;
    return [u.path.lowercaseString containsString:@"/bookkeeping/user/getUserInfoById"];
}
